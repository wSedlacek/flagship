...
export default class EngagementProductModal extends PureComponent<
  EngagementProductModalProps,
  EngagementProductModalState
> {
  navigator: Navigator;
  ...
  componentDidMount(): void {
    if (this.AnimatedModal) {
      this.AnimatedModal.transition({ opacity: 0 }, { opacity: 1 }, 300, 'linear');
    }
  ...
  onScrollCarouselItem = (event: any) => {
    ...
    if (this.scrollPosition > 15 && !this.zoomedIn) {
      if (this.AnimatedContent) {
        ...
        this.AnimatedContent.transitionTo({ translateY: -30, scale: 1.08 }, 400, 'ease-out');
      }
    } else if (this.scrollPosition <= 15 && this.zoomedIn) {
      if (this.AnimatedContent) {
        this.zoomedIn = false;
        this.setState({ scrollEnabled: true });
        this.AnimatedContent.transitionTo({ translateY: 0, scale: 1 }, 400, 'ease-out');
      }
    }
  };
    ...
    return (
      <View style={styles.slideInnerContainer}>
        <View style={[styles.itemContainer]}>
          ...
          >
            <Image resizeMode="contain" source={simpleCloseIcn} style={styles.simpleCloseIcn} />
          </TouchableOpacity>
          <ScrollView onScroll={this.onScrollCarouselItem} scrollEventThrottle={16}>
            <View style={styles.topContentContainer}>
              <Image source={item.image.source} style={imageStyle} />
              ...
              <View style={styles.starContainer}>
                <View style={styles.alignStart}>
                  ...
                  <Text style={{ fontSize: 12, color: '#999' }}>{item.reviewCount} Ratings</Text>
                </View>
              ...
              <View style={styles.buttonContainer}>
                ...
                >
                  <View style={styles.alignCenter}>
                    <Text style={styles.buttonText}>BUY | {item.price}</Text>
                  </View>
                </TouchableOpacity>
              </View>
              <View style={styles.secondaryButtonContainer}>
                <View style={styles.alignStart}>
                  <Text style={{ fontSize: 12, color: '#777' }}>Available to ship now</Text>
                </View>
              ...
              <View style={{ paddingBottom: 30 }}>
                ...
                <Text style={styles.regularText}>{item.productInfo}</Text>
                ...
                <Text style={styles.regularText}>{item.features}</Text>
                ...
                >
                  <View style={{ flex: 1, justifyContent: 'center' }}>
                    <Text style={styles.categoryHeader}>Customer Reviews</Text>
                  </View>
                  <View style={styles.alignEnd}>
                ...
                {item.review && (
                  <View style={styles.reviewContainer}>
                    <Text style={styles.reviewTitle}>Awesome Product!</Text>
                    <Text style={styles.reviewText}>{item.review}</Text>
                    <View style={{ marginTop: 5, flexDirection: 'row', alignItems: 'center' }}>
                      <Image source={stars} style={styles.starsSmall} />
                      <Text style={styles.reviewDate}>Dec 5, 2019</Text>
                    </View>
                  </View>
                )}
              </View>
  ...
  closeModal = () => {
    ...
    setTimeout(() => {
      if (this.AnimatedModal) {
        this.AnimatedModal.transition({ opacity: 1 }, { opacity: 0 }, 300, 'linear');
      }
    }, 300);
    ...
    return (
      <View style={styles.growStretch}>
        ...
        <Animatable.View
          ...
          style={{
            marginTop: 50,
            transform: [{ translateY: viewportHeight }],
          }}
        >
          <Carousel
            data={products || []}
